...
func TestTickerConfigurator(t *testing.T) {
	...
	tickerConfTests := []tickerConfigTests{
		...
		{time.Duration(30 * time.Second), 0, 0}, // stop -> invalid duration, no change
		{time.Duration(0), 0, 0},                // still stopped -> stop, no change
		{time.Duration(2 * time.Minute), 1, 0},  // still stopped -> start, expect start
	}
